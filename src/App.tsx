...
export default function App() {
  ...
  const handlePostEASubmit = (data: any) => {
    ...
    // Automatically approve the provider when they post their EA so they enter the web app immediately without waiting
    if (currentUserEmail) {
  ...
  const handleSelectBroker = (brokerName: string, shouldNavigate: boolean = false) => {
    ...
    const updatedUsers = users.map(u => {
      if (u.email === currentUserEmail) {
        // Automatically approve the user immediately when selecting a broker (or personal account), entering the app directly
        return { ...u, preferredBroker: brokerName, isApproved: true };
      }
      return u;
    });
    setUsers(updatedUsers);
              ...
              <div className="p-12 bg-[#111] border border-white/5 rounded-[48px] shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-end gap-6 p-6 bg-white/5 border border-white/5 rounded-3xl">
                    <button 
                      onClick={() => {
                        // Automatically approve user in case they skipped selecting a broker to avoid blocking them
                        if (currentUserEmail) {
                          const updatedUsers = users.map(u => {
                            if (u.email === currentUserEmail) {
                              return { ...u, isApproved: true };
                      ...
                      className="text-[10px] font-black text-yellow-500 hover:text-yellow-400 uppercase tracking-widest transition-colors flex items-center gap-2"
                    >
                      เสร็จสิ้นและเข้าใช้งานได้ทันที (Done & Enter Platform)
                      <ArrowRight className="w-3 h-3" />
                    </button>
                  </div>
                </div>
